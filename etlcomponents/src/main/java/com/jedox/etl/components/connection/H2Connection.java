...
public class H2Connection extends RelationalConnection {
	...
	public synchronized java.sql.Connection open() throws RuntimeException {
		...
		try {
			//if(!connection.createStatement().executeQuery("SELECT * FROM INFORMATION_SCHEMA.FUNCTION_ALIASES where ALIAS_NAME='FIRST'").next())
				connection.createStatement().execute("CREATE AGGREGATE IF NOT EXISTS FIRST FOR \"com.jedox.etl.components.connection.extensions.H2AggregateFirst\"");
		}
		catch (Exception e) {}
		try {
			//if(!connection.createStatement().executeQuery("SELECT * FROM INFORMATION_SCHEMA.FUNCTION_ALIASES where ALIAS_NAME='LAST'").next())
				connection.createStatement().execute("CREATE AGGREGATE IF NOT EXISTS LAST FOR \"com.jedox.etl.components.connection.extensions.H2AggregateLast\"");
		}
		catch (Exception e) {}
		return connection;
	}
