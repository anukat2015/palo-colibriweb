...
public class SQLExtract extends TableSource implements IRelationalExtract {
	...
	public Row getOutputDescription() throws RuntimeException {
		...
		log.debug("Query: "+((query==null)?"":query));
		try {
	...
	protected IProcessor getSourceProcessor(int size) throws RuntimeException {
		...
		log.debug("Query: "+((query==null)?"":query));
		try {
				IProcessor processor = initProcessor(getConnection().getProcessor(query, getAliasMap(),false, true, size),Facets.OUTPUT);
	...
	public void init() throws InitializationException {
		super.init();
		try {
			if(getConnection() instanceof IFileConnection && !(this instanceof FileExtract)){
				throw new InitializationException("File connections can only be used in extracts from type \"File\"");
			}
