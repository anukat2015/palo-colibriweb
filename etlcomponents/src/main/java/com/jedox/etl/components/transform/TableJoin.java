...
public class TableJoin extends TableSource implements ITransform {
	...
	private CacheTypes persist = CacheTypes.none;
	...
	protected IProcessor getSourceProcessor(int size) throws RuntimeException {
		CacheTypes myCacheType = persist;
	...
	private CacheTypes getPersistence(List<JoinDefinition> definitions) {
			...
			for (JoinDefinition d : definitions) {
				for (Match m : d.getMatches()) {
					if (!m.getCondition().equals(Conditions.EQ)) {
						log.debug("Join Condition "+m.getCondition().toString()+" in transform "+getName()+" needs persistence. Switching to memory join.");
						return CacheTypes.memory; //so an in memory sql join
					}
				}
				if (d.getType().equalsIgnoreCase("full")) {
					log.debug("Full join in transform "+getName()+" needs persistence. Switching to memory join.");
					return CacheTypes.memory; //do an in memory sql join
				}
			}
			return CacheTypes.none; //do Lookup Join
		}
		return CacheTypes.disk; //do persistent disk join
	}
