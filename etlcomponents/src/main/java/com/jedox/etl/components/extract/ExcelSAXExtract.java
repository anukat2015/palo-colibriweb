...
public class ExcelSAXExtract extends ExcelExtract implements IExtract {
	...
	private class ExcelProcessor extends Processor {
		...
		protected void init() throws RuntimeException {
			if (parser==null) {
				log.info("Parsing Excel worksheet...");
			...
			if (header) {
				...
				for(int i=0;i<headerRow.size();i++){
					if(map.contains(headerRow.get(i).toString()))
						throw new RuntimeException("Column " + headerRow.get(i).toString() + " exists in the header row more than once.");
					map.add(headerRow.get(i).toString());
				}
			} else {
