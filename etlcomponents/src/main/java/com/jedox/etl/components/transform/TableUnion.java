...
public class TableUnion extends TableSource implements ITransform {
	...
	private boolean typeWarnings = false;
	...
	protected IProcessor getSourceProcessor(int size) throws RuntimeException {
		...
		if (!loopManager.isEmpty()) { //there are loops
			IProcessor loops = initProcessor(UnionProcessor.getInstance(loopManager.getProcessors(),typeWarnings),Facets.HIDDEN);
			...
			IProcessor unionProcessor = initProcessor(UnionProcessor.getInstance(baseManager.getProcessors(),typeWarnings),Facets.INPUT);
	...
	public void init() throws InitializationException {
		...
		try {
			...
			typeWarnings = getConfigurator().getParameter("warnings","false").equalsIgnoreCase("true");
		}
		catch (Exception e) {
