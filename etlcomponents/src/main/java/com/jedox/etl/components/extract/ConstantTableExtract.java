...
public class ConstantTableExtract extends TableSource implements IExtract {

	private Element data;	

	public ConstantTableExtract() {
		setConfigurator(new TableSourceConfigurator());
	}

	protected IProcessor getSourceProcessor(int size) throws RuntimeException {
		IProcessor processor = initProcessor(new ConstantTableProcessor(data, size),Facets.OUTPUT);
		return processor;
	}
	
	public Row getOutputDescription() throws RuntimeException {		
		return getSourceProcessor(1).current();
	}
