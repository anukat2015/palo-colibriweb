...
public class RTransform extends TableSource implements ITransform  {
	...
	private void checkRInstallation() throws Exception {
		String rPath=new File(System.getenv("R_HOME")).getCanonicalPath();
		if (rPath==null)
		...
		if (model==null) { 
			...
			if (!FileUtil.fileExistsInSystemPath(pathToR))
				throw new ConfigurationException("The environment variable PATH does to include the path to R: "+pathToR);
		}	
