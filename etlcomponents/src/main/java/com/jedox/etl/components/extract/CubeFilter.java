...
public class CubeFilter {
	...
	public DimensionFilter addDimensionFilter(String dimension,DimensionFilterDefinition dfd) throws RuntimeException {
		if (filters.containsKey(dimension)) {
			throw new RuntimeException("Duplicate filter for Dimension "+dimension+" in Cube "+cubeName+".");
		}
		...
		for (String c : dfd.getFieldNames()) {
			if (!c.equals(NamingUtil.getElementnameElement())) 
				withAttributes = true;
		}
