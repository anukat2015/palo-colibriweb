...
public class SqliteConnection extends RelationalConnection {
	...
	protected String getDatabaseString() {
		if (super.getDatabaseString().startsWith("$"))
			return super.getDatabaseString().substring(1)+".db";
...
*/	
	public void init() throws InitializationException {
		...
		if (FileUtil.isRelativ(getDatabaseString()) && getConnectionParameters().getProperty(NamingUtil.internal("drop"), "false").equalsIgnoreCase("true")) {
			FileUtil.deleteInDirectory(new File(getDataDir()),"[\\w\\s-\\d]+.db$");
			FileUtil.deleteInDirectory(new File(getDataDir()),"[\\w\\s-\\d]+journal$");
		}
	}
