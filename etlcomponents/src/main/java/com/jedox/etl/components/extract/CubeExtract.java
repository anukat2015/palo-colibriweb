...
public class CubeExtract extends TableSource implements IExtract {
	...
	private class CubeProcessor extends Processor {
		...
		public CubeProcessor(CubeFilter filter) {
			this.filter = filter;
		}
				...
				try {
					...
					if (cell != null) {
						...
						if (path != null) {
							...
							//value
							String stringValue  = getStringValue(cell.getValue());
							row.getColumn(row.size()-1).setValue(stringValue);
							
							if(stringValue.equals("not authorized for operation"))
								aggLog.info("Some cells can not be read since there is no sufficient authorization for the user");
	...
	protected IProcessor getSourceProcessor(int size) throws RuntimeException {
		IProcessor result;
		if (drillthrough) {
			result = initProcessor(getDrillthroughProcessor(size), Facets.OUTPUT);
		}
		else {
			result = initProcessor(new CubeProcessor(getCubeFilter()),Facets.OUTPUT);
