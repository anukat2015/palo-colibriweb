...
public class CalendarExtract extends TreeSource implements IExtract {
	...
	private Levels getRootLevel() {		
		switch (getLevels().get(0).getName()) {
			case hours : return Levels.days;
			case minutes : return Levels.hours;
			case seconds : return Levels.minutes;
			default: return Levels.years;
		}
	}
	...
	public ITreeProcessor buildTree() throws RuntimeException {
		...
		manager.addAttributeValue(levelAttribute, rootElement.getName(), getRootLevel().toString());

		//generate Levels
		generateYears(getLevel(Levels.years));
		generateHalfYears(getLevel(Levels.halfyears));
	...
	private ITreeElement addNode(Calendar calendar, LevelDescriptor level, ITreeElement parent) {
		...
		for (LevelAttribute a : level.getAttributes()) {
			...
			if (manager.getAttributeByName(a.getName())==null) {
				manager.addAttribute(a.getName(),ElementType.ELEMENT_STRING).setMode(a.getType());	
			}
	...
	private boolean getCondition(ITreeElement parent, Calendar calendar) {
		...
		case halfyears: return parentCalendar.get(Calendar.YEAR) == calendar.get(Calendar.YEAR) && parentCalendar.get(Calendar.MONTH)+6 > calendar.get(Calendar.MONTH);
		...
		case weeks: return parentCalendar.get(Calendar.WEEK_OF_YEAR) == calendar.get(Calendar.WEEK_OF_YEAR) && parentCalendar.get(Calendar.YEAR) == calendar.get(Calendar.YEAR) && ((!existsLevel(Levels.months)&&!existsLevel(Levels.quarters)&&!existsLevel(Levels.halfyears)) || parentCalendar.get(Calendar.MONTH) == calendar.get(Calendar.MONTH));
	...
	private void generateHalfYears(LevelDescriptor halfyears) {
		if (halfyears != null) {
			Calendar c = getCalendar(halfyears);
			ITreeElement lastNode = null;
			ITreeElement e = null;
			ArrayList<ITreeElement> newLeaves = new ArrayList<ITreeElement>();
			for (ITreeElement parent : uniqueLeaves) {
				...
				for (int i=1; getCondition(parent,c); i++) {
					if (i >= halfyears.getStart() && i <= halfyears.getEnd()){
						e = addNode(c,halfyears,parent);
						manager.addAttributeValue(timeAttribute, e.getName(), c.getTime());
						newLeaves.add(e);
						if(checkTTDPattern(halfyears)){
							lastNode = getElementAgg(e, lastNode, halfyears);
						}
					}
	...
	public void init() throws InitializationException {
		try {
			...
			if (getOptions().getCompleteWeek() && existsLevel(Levels.weeks) && (existsLevel(Levels.months)||existsLevel(Levels.quarters)||(existsLevel(Levels.halfyears))))
