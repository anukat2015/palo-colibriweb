...
public class DimensionFilterDefinition extends Conditions {
	...
	private boolean readSingleElements = true;
	...
	public TreeCondition addTreeCondition(String mode, IEvaluator evaluator ,FilterModes m, String fieldName) {
		if(!(evaluator instanceof EqualityEvaluator) || !m.equals(FilterModes.onlyNodes) || !mode.equals("accept") || !fieldName.equals(NamingUtil.getElementnameElement()))
			readSingleElements = false;
	...
	public boolean isReadSingleElements() {
		return readSingleElements && getTreeConditions().size() > 0;
	}
}
