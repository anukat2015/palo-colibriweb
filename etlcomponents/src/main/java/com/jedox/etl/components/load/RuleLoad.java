...
public class RuleLoad extends AbstractOlapLoad implements ILoad {
	...
	private IRule getRuleFromRow (Row r) throws RuntimeException {		
		...
		String timeStampValue = r.getColumn(3).getValueAsString();
		long timeStamp;
		try {
			timeStamp=Long.parseLong((timeStampValue.isEmpty()?"0":timeStampValue));
		}
		catch (NumberFormatException e) {
			throw new RuntimeException("Invalid rule timestamp "+timeStampValue);
		}
