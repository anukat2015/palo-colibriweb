...
public class RegExp extends Function {
	...
	protected Object transform(Row inputs) {
		String value = inputs.getColumn(0).getValueAsString();
		return value.toString().replaceAll(regexp, replacement);
	}
	
	protected void validateParameters() throws ConfigurationException {
		regexp = getParameter("regexp","");
		replacement = Matcher.quoteReplacement(getParameter("replacement","").replaceAll(NamingUtil.spaceValue(), " "));
	}
