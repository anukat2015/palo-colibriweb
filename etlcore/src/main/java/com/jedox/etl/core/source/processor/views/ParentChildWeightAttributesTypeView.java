...
public class ParentChildWeightAttributesTypeView extends ParentChildWeightAttributesView {
	...
	protected CoordinateNode getTypeCoordinate() {
		CoordinateNode c = ColumnNodeFactory.getInstance().createCoordinateNode(NamingUtil.getElementnameNodeType(),null);
		return c;
	}
	...
	public Row fillRow(int current){
		...
		ITreeElement node = getTree().getElement(getTargetRow().getColumn(1).getValueAsString());
