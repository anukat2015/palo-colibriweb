...
public class ExecutionLog extends WriterAppender {
	...
	public static PatternLayout layout = new PatternLayout("%d %5p : %m%n");
...
//		previous layout: "%d %5p [%t] (%F:%L) - %m%n"
		super(layout, new java.io.StringWriter());
		this.result = result;
	...
	public void close() {
		if (!isClosed) {
			...
			if (result.getMessageWriter() != null)
				result.getMessageWriter().flush();
		}
		//super.close();
	}
