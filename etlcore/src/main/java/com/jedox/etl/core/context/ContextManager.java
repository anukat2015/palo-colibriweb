...
public class ContextManager extends Manager {
	...
	public void provideDefault() throws CreationException {
		try {
			...
			context.setConfigManager(ConfigManager.getInstance()); //set global configManager for default context
			add(context);
		}
		catch (Exception e) {
	...
	public synchronized IContext provide(IContext parentContext) throws CreationException, RuntimeException {
		...
		if (parentContext != null) {
			...
			//spawn local read only runtime config manager to protect runtime context from changes in global config during executions
			childContext.setConfigManager(parentContext.getConfigManager().spawnReadOnlyInstance(parentContext.getLocator().getRootLocator()));
			if (!parentContext.isDefault()) parentContext.addChildContext(childContext); //cschw: do not append to default project context, since it will not be used or cleared there
			childContext.setBaseContextName(parentContext.getName());
			childContext.getVariables().putAll(parentContext.getVariables());
			childContext.setState(parentContext.getState());
			add(childContext);
			return childContext;
		} 
		else
	...
	private void removeContextFromParent(IContext context) {
		if (context != null) {
			IContext parent = get(context.getBaseContextName());
			if (parent != null) {
				parent.getManager(ITypes.Contexts).remove(context.getName());
			}
		}
	}
	...
	public IContext remove(String name) {
		...
		if (result != null && result.getManagers().length > 0) { //cschw: also remove child contexts. 
			ContextManager ccm = (ContextManager) result.getManager(ITypes.Contexts);
			if (ccm != null) for (IContext c : ccm.getAll()) {
				remove(c.getName());
			}
			ccm.clear();
		}
		removeContextFromParent(result);
		return result;
	}
