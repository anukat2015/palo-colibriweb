...
public class UnionProcessor extends Processor {
	...
	public static IProcessor getInstance(List<IProcessor> processors) throws RuntimeException {
		return getInstance(processors,true);
	}
	
	public static IProcessor getInstance(List<IProcessor> processors, boolean typeWarnings) throws RuntimeException {
		if (processors.size() == 0)
			throw new RuntimeException("Nothing to be processed.");
		if (processors.size() == 1)
			return processors.get(0);
		UnionProcessor p = new UnionProcessor(processors);
		p.setTypeWarnings(typeWarnings);
		return p;
	}
	...
	private void setTypeWarnings(boolean typeWarnings) {
		...
		for (int i=0; i<processors.size(); i++) {
			...
			for (IColumn source : p.current().getColumns()) {
				...
				if (target == null) { //column is not yet present in internal row
					...
					if (!target.getValueType().equals(source.getValueType()) && typeWarnings)
