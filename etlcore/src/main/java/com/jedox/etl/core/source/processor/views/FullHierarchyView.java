...
public class FullHierarchyView implements IModelView {
	...
	public FullHierarchyView(ITreeManager manager) {
	...
	protected Row initRow(int length) {
		...
		for (int i=0; i<length; i++) {
			...
			CoordinateNode coordinate = ColumnNodeFactory.getInstance().createCoordinateNode("level"+(i+1),null);
	...
	public Row setupRow(Row attributeRow) throws RuntimeException {
		...
		row = initRow(maxLength);
		return row;
	}
	
	public void init(ITreeManager manager) {
