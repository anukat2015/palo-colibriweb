...
*/	
	...
	public static String readStreamAsString(InputStream in) throws java.io.IOException {
	 ...
	 public static boolean fileExistsInSystemPath(String filename) throws IOException {

		String filenameCanonical;
		filenameCanonical = new File(filename).getCanonicalPath();
		String systemPath = System.getenv("PATH");
		if (systemPath==null)
			return false;

		for (String path : systemPath.split(";")) {
			if (new File(path).getCanonicalPath().equals(filenameCanonical)) {
				return true;
			}					
		}
		return false;
