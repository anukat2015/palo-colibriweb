...
public class TableSourceConfigurator extends BasicConfigurator {
	...
	public IConnection getConnection() throws ConfigurationException {
		...
		if (connection != null) {
			...
			return (IConnection)getContext().getComponent(getLocator().getRootLocator().add(ITypes.Connections).add(connectionID));
		}
		return null;
	}
...
//	}
	...
	public CacheTypes getCacheType() {
		CacheTypes result = CacheTypes.none;
		String value = getXML().getChildTextTrim("cache");
		if (value == null) {
			try {
				value = getParameter("cache",result.toString());
			}
			catch (ConfigurationException e) {}
		}
		if (value != null) {
			try {
				result = CacheTypes.valueOf(value);
