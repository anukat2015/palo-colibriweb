...
public class ConnectionFactory {
	...
	public IConnection newConnection(ComponentDescriptor descriptor, ILocatable parent, IContext context, Element config) throws CreationException {
		try {
			...
			String mode = config.getChildText("mode");
			c.setDriver(descriptor.getDriver(mode));
			c.setProtocol(descriptor.getJDBC(mode));
			...
			c.getConfigurator().setXML(config, resolver.getVariables());
			c.initialize();
			return c;
		}
		catch (Exception e) {
