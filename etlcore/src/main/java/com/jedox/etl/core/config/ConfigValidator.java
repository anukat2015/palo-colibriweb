...
public class ConfigValidator {
	...
	/*private class SchemaResourceResolver implements LSResourceResolver {
		...
		private SchemaResourceResolver(Class<?> className) throws ClassNotFoundException,
		...
		public LSInput resolveResource(String type, String namespaceURI,
			...
			InputStream is = null;
			try {
				is = getSchemaResourceDocument(className);
			} catch (Exception e) {
	...
	public synchronized Document getSchemaDocument(Class<?> implementingClass) throws SAXException, IOException {
		String className = implementingClass.getCanonicalName();
		Document schemaDoc = documents.get(className);
		if (schemaDoc == null) {
			//get the main schema as stream
			...
			//get resource.xsd as stream and add its content to the schema if it was included
			// remove the include
			// needed for the getComponentsTypes
			InputStream ris = getSchemaResourceDocument(implementingClass);
			if (ris != null){
				Document schemaResourceDoc = parser.parse(ris);
				if(schemaDoc.getElementsByTagName("include").getLength()!=0){
					schemaDoc.getDocumentElement().removeChild(schemaDoc.getElementsByTagName("include").item(0));
					NodeList children = schemaResourceDoc.getDocumentElement().getChildNodes();
					for(int i=0; i<children.getLength();i++){
						//log.info(children.item(i).getNodeName());
						Node node = children.item(i).cloneNode(true);
						Node importedNode = schemaDoc.importNode(node,true);
						schemaDoc.getDocumentElement().appendChild(importedNode);
					}
				}
			}
	...
	/*public synchronized Document getMergedSchemaDocument(Class<?> implementingClass) throws SAXException, IOException {

		Document schemaDoc = getSchemaDocument(implementingClass);
		//get resource as stream
		InputStream is = getSchemaResourceDocument(implementingClass);
		if (is != null){
			Document schemaResourceDoc = parser.parse(is);
			schemaDoc.getDocumentElement().removeChild(schemaDoc.getElementsByTagName("include").item(0));
			NodeList children = schemaResourceDoc.getDocumentElement().getElementsByTagName("simpleType");
			for(int i=0; i<children.getLength();i++){
				log.info(children.item(i).getNodeName());
				Node node = children.item(i).cloneNode(true);
				Node importedNode = schemaDoc.importNode(node,true);
	...
	public synchronized InputStream getSchemaResourceDocument(Class<?> implementingClass) throws SAXException, IOException {
		//get ressource as stream
		InputStream is = implementingClass.getResourceAsStream("resource.xsd");
		if (is == null) //fall back to default
			return null;
		else //parse document from stream
			return is;
	...
	private Validator getValidator(Class<?> implementingClass) throws SAXException, IOException {
		...
		if (validator == null) {
			...
			SchemaFactory sf = SchemaFactory.newInstance("http://www.w3.org/XML/XMLSchema/v1.1");
			/*try {
				if(getSchemaResourceDocument(implementingClass)!=null)
					sf.setResourceResolver(new SchemaResourceResolver(implementingClass));
			} catch (Exception e) {
	...
	public String getSchemaURI(Class<?>implementingClass) {
		Document d;
		String uri = null;
		try {
		...
		if (uri == null)
			uri = "http://schemas.jedox.com/etlcore";
		return uri;
	}
	
	private String jdomToString(Element element, Class<?> implementingClass) {
		//Element clone = (Element)element.clone();
		//clone.setNamespace(Namespace.getNamespace(getSchemaURI(implementingClass)));
		String configString = "";
		try {
