...
public class NCAttributesView extends NCView {
	...
	public Row fillRow(int current) {
		...
		for (int i=0; i<attributes.getAttributes().size(); i++) {
			attributes.fillAttribute(getElements().get(current),i,getTree());
		}
		return getTargetRow();
	}
	...
	public Row setupRow(Row attributeRow) throws RuntimeException {
		...
		//now add aliases and attributes to row
		getTargetRow().addColumns(attributes.getAttributes());
		return getTargetRow();
	}
