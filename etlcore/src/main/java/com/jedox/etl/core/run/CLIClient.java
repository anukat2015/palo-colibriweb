...
public class CLIClient {
	...
	public CLIClient(String[] args) {
		...
		previewopt.setArgs(2);		
		Option dataformatopt = new Option("df", "dataformat", true,
		"format of data from a source to csv or xml. Valid only in combination with the -d option");
		previewopt.setArgName("format");
		previewopt.setArgs(1);		
		...
		Option prototype = new Option("pg","prototype",true,"Generate a prototype project");
	...
	/**
	 * gets the specified data source output format (used with -d option value)
	 * @return the specified data source
	 */
	public String getDataFormat() {
		return cli.getOptionValue("dataformat");
	}
	...
	public int getSample() {
		String preview;
		if (cli.getOptionValues("sample")==null)
			preview="0";
		else
			preview=cli.getOptionValues("sample")[0];
		int sample = 0;
		try {
	...
	public int getOffset() {
		String preview;
		if (cli.getOptionValues("sample")==null || cli.getOptionValues("sample").length==1)
			preview="0";
		else
			preview=cli.getOptionValues("sample")[1];
		int offset = 0;
		try {
			offset = Integer.parseInt(preview);
		} catch (Exception e) {
			System.err.println("Preview Offset Size is not a number: "
					+ preview);
		}
		return offset;
	}
	...
	public Properties getContext() {
		...
		if (parameter != null)
			for (String p : parameter) {
				...
				int index = p.indexOf("=");
				if(index!=-1){
					pair[0]=p.substring(0, index);
					pair[1]=p.substring(index+1);
	 ...
	 * @return
	 */
	public String getLogLevel(String defaultLevel) {
		return cli.getOptionValue("loglevel", defaultLevel);
	}	
	...
	/**
	 * Determines, if the prototype project should be generated
	 * @return
	 */
	public boolean doPrototype() {
		return cli.hasOption("prototype");
	}	
	/**
