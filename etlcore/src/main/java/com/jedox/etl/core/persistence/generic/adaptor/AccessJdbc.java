...
public class AccessJdbc extends GenericPersistor {
	...
	public AccessJdbc(IConnection connection) {
	...
	public String getPersistentName(Locator locator) throws RuntimeException {
		return escapeName(locator.getPersistentTable());
	...
	protected Hashtable<String, String> getLookUp() {
		...
		lookup.put(java.util.Date.class.getCanonicalName(), "DATE/TIME");
		lookup.put(java.sql.Date.class.getCanonicalName(), "DATE/TIME");
	...
	protected void fillStatement(PreparedStatement statement, IColumn column, int pos) throws RuntimeException {
		try {
			...
			if(column.getValueType().equals(Double.class) && !localName.equals("")){
