...
public class ConfigConverter {
	...
	private boolean verifySmcConnection;
	private boolean verifySmcRelConnection;
	...
	private enum Functions {
		Date2Dim, DateFormat, Aggregation, Concatenation, Java, JavaScript, Groovy, Map, Replace, Lookup, SubString, UpperLower, NumberFormat, smcScript
	}
	...
	private boolean fixSource(Element source) {
		...
		if (source.getAttribute("nameref") == null) { //ignore references
			...
			*/
			...
			//opensmc extension package
			//conversion for existing cache tags
			if (source.getChildTextTrim("cache") != null) {
				if (source.getChildTextTrim("cache").equalsIgnoreCase("true")) {
					source.getChild("cache").setText("memory");
				}
				if (source.getChildTextTrim("cache").equalsIgnoreCase("false")) {
					source.getChild("cache").setText("none");
				}
			}
			if (source.getAttributeValue("type").equalsIgnoreCase("smcExtract")) {
				if (source.getChild("connection") == null) {
					Element connection = new Element("connection");
					connection.setAttribute("nameref", "smcConnection");
					source.addContent(connection);
					verifySmcConnection = true;
				}
			}
			if (source.getAttributeValue("type").equalsIgnoreCase("smcQuery")) {
				source.setAttribute("type", "Relational");
				if (source.getChild("connection") == null) {
					Element connection = new Element("connection");
					connection.setAttribute("nameref", "smcRelational");
					source.addContent(connection);
					verifySmcRelConnection = true;
				}
			}
	...
	private boolean fixPipeline(Element pipeline) {
		...
		//opensmc extension package
		 if(pipeline.getAttributeValue("type").equals("TableUnion")){
	        if (pipeline.getChild("suppressWarnings") != null) {
	        	pipeline.getChild("suppressWarnings").setName("warnings");
	       }
	     }
		...
		if (pipeline.getAttributeValue("type").equals("TreeFH")) {
			if (target != null) {
				Element levels = target.getChild("levels");
				Set<String> attributeNames = new HashSet<String>();
				if (levels != null) {
					for (Object o : levels.getChildren("level")) {
						Element level = (Element)o;
						Element attributes = level.getChild("attributes");
						if (attributes != null) {
							for (Object a : attributes.getChildren()) {
								Element attribute = (Element)a;
								attributeNames.add(attribute.getAttributeValue("name"));
				...
				if (!attributeNames.isEmpty()) {
					Element attributes = target.getChild("attributes") ;
					if (attributes == null) { 
						attributes = new Element("attributes");
						target.addContent(attributes);
					}
					for (Object o : attributes.getChildren()) { //remove existing from todo list
						Element attribute = (Element)o;
						attributeNames.remove(attribute.getAttributeValue("name"));
					}
					for (String n : attributeNames) {
						Element attribute = new Element("attribute");
						attribute.setAttribute("name", n);
						attribute.setAttribute("type", "string");
		...
		//Colibri: Normalisation measure name removal
		if(pipeline.getAttributeValue("type").equals("TableNormalization")){
			if (target != null) {
				Element measures = target.getChild("measures");
				if (measures != null) {
					@SuppressWarnings("unchecked")
	...
	private boolean fixTransformer(Element transformer) {
		...
		try {
	        ...
	        //for opensmc package
	        if(transformer.getAttributeValue("type").equals("smcScript")){
	        	Element scriptElement = transformer.getChild("parameters").getChild("script");
	        	String script = scriptElement.getTextTrim();
	        	transformer.setAttribute("type", "Groovy");
	        	if (script.contains("SESSION")) {
	        		script = "SESSION = API.getConnection('smcConnection').open().getDelegate();\n" + script;
	        	}
	        	if (script.contains("ENTITIES")) {
	        		script = "ENTITIES = API.getConnection('smcConnection').open();\n" + script;
	        	}
	        	scriptElement.setContent(new CDATA(script));
	        }
		}
		catch (Exception e) {}
		return true;
	...
	private boolean fixExport(Element export) {
		...
		//opensmc extension package
		if (export.getAttributeValue("type").equalsIgnoreCase("smcLoad")) {
			if (export.getChild("connection") == null) {
				Element connection = new Element("connection");
				connection.setAttribute("nameref", "smcConnection");
				export.addContent(connection);
				verifySmcConnection = true;
			}
		}
		//jedox compat: cube load cube element
		if (export.getAttributeValue("type").equalsIgnoreCase("Cube")) {
			if (export.getChild("cube") == null) {
				Element cube = new Element("cube");
				cube.setAttribute("name", export.getAttributeValue("name"));
				export.addContent(cube);
			}
		}
		return true;
	}
	...
	private boolean fixJob(Element job) {
		...
		//opensmc extension package
        if(job.getAttributeValue("type").equals("smcScript")){
        	Element scriptElement = job.getChild("script");
        	scriptElement.setName("jobscript");
        	String script = scriptElement.getTextTrim();
        	job.setAttribute("type", "Groovy");
        	if (script.contains("SESSION")) {
        		script = "SESSION = API.getConnection('smcConnection').open().getDelegate();\n" + script;
        ...
        //convert standard job with script tag
        if (job.getAttributeValue("type").equalsIgnoreCase("Standard") && job.getChildText("script") != null) {
        	Element scriptElement = job.getChild("script");
        	scriptElement.setName("jobscript");
        	job.setAttribute("type", "Groovy");
        }
...
*/		
	...
	private boolean findByName(Element manager, String name) {
		for (Object o : manager.getChildren()) {
			Element e = (Element)o;
			if (e.getAttributeValue("name").equalsIgnoreCase(name)) return true;
		}
		return false;
	}
	
	private void verifySMCConnections(Element project) {
		Element connections = project.getChild("connections");
		if (connections == null) {
			connections = new Element("connections");
			project.addContent(connections);
		}
		if (verifySmcConnection) {
			if (!findByName(connections,"smcConnection")) {
				Element c = new Element("connection");
				c.setAttribute("type", "smcConnection");
				c.setAttribute("name", "smcConnection");
				...
				c.addContent(password);
				Element database = new Element("database");
				database.setText("opensmc");
				c.addContent(database);
				connections.addContent(c);
			}
		}
		if (verifySmcRelConnection) {
			if (!findByName(connections,"smcRelational")) {
				Element c = new Element("connection");
				c.setAttribute("type", "Postgresql");
				c.setAttribute("name", "smcRelational");
				Element host = new Element("host");
				host.setText("localhost");
				c.addContent(host);
				Element port = new Element("port");
