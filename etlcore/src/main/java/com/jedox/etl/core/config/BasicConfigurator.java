...
public class BasicConfigurator extends Locatable implements IConfigurator {
	...
	// EXPERIMENTAL AND TO BE DISCUSSED
	@SuppressWarnings("unchecked")
	public  <T extends Object> T getConfigValue(String childName,String attName,
			Class<?> className, boolean paramterFallBack,T defaultValue) throws ConfigurationException {
		String value = null;
		if(attName==null)
			value = getXML().getChildTextTrim(childName);
		else{
			Element child = getXML().getChild(childName);
			if(child!=null)
				value = child.getAttributeValue(attName);
		}
		
		// try to get the value from the parameters list
		if(paramterFallBack){
			if(value == null && getXML().getChild("parameters") != null){
				value = getXML().getChild("parameters").getChildTextTrim(childName);
			}
			//try to get from a parameter field
			if (value == null)
				value = getParameter().getProperty(childName);
		}
			
		if (value != null) {
			if (String.class == className)
				return (T) String.valueOf(value);
			if (Boolean.class == className)
				return (T) Boolean.valueOf(value);
			if (Integer.class == className)
				return (T) Integer.valueOf(value);
			if (Long.class == className)
				return (T) Long.valueOf(value);
			if (Float.class == className)
				return (T) Float.valueOf(value);
			if (Double.class == className)
				return (T) Double.valueOf(value);
			if (Short.class == className)
				return (T) Short.valueOf(value);
			throw new ConfigurationException("no supported type.");
		}
